...
export default function ContactPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (formData.contactMethod === 'discord' && !formData.discord) {
        throw new Error('Please provide your Discord username');
      }
      
      // Create API request body - Keep original format
      const requestBody = {
        name: formData.name,
      ...
      setFormStatus({ success: false, message: error.message || 'Failed to send message. Please try again.' });
    } finally {
            ...
            >
              <div className="bg-gray-800 rounded-lg shadow-xl p-8">
                ...
                <div className="mt-12 p-6 bg-gray-700 rounded-lg border border-gray-600">
                  ...
                  <p className="text-gray-300">Average response time: 4-8 hours</p>
                </div>
              </div>
            </motion.div>
          </div>
